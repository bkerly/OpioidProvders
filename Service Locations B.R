...
docs<-read.csv("C:/Users/sje0303/Downloads/co_opioid_services.csv")
docs2<-docs[,c(1,10:11)]%>%distinct()
...
for (i in c(1:nrow(dthloc))){
  ...
  dist5<-as.data.frame(unlist(dist4[1])[2:26])%>%rename(dist=1)
  dist6<-dist2[2:26,]
  dist6$dist<-dist5
  
  dist5b<-dist4[["durations"]][2:26,1]
  dist5a<-dist4[["durations"]][1,2:26]
  
  dist8<-as.data.frame(cbind(dist5a,dist5b))%>% rowwise()%>%mutate(time=(min(dist5a,dist5b)))%>%select(time)%>%
    as.data.frame()
  dist8$id<-dist2[2:26,3]
...
result<-data.frame("Baseline",sum(out<20.001)/nrow(frame3),stringsAsFactors = F)  #Frame for Results to stack onto
...
for (k in 1:20) {
  ...
  for (l1 in 1:nrow(minim)) {
    ...
    Ncover[l1]<-sum(minim[l1,1:deathcount]<20.001)
  }
  ...
  prints<-frame_a[,3]%>% cbind(minim[,c("Time_Saved", "Ncover", "New_Distance")])%>%as.data.frame()%>%arrange(New_Distance)%>%
    rename(GEOID=1)%>% filter(New_Distance==min(New_Distance))
  ...
  out<-minim %>% filter(New_Distance==min(New_Distance))%>% arrange(New_Distance)
  out<-out[1,1:deathcount]
  
  
  result[k+1,1]<-as.character(head(prints$GEOID,1))
  result[k+1,2]<-sum(out[1,1:deathcount]<20.0001)/nrow(frame3)
...
outpl<-minim%>% cbind(as.data.frame(geo4[,c(1:3)]))%>%
  filter(id %in% target)
data<-as.data.frame(outpl[,385:387])
